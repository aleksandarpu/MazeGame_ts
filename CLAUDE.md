...
**Sounds** — `sounds.ts` preloads `assets/sound/*.wav`; `playSound(name)` plays a copy each time, so sounds can overlap. Triggers: roll (while the die tumbles in `dice-pop-up.ts`; faded out with `fadeOutSound` when it settles), footstep (valid move), damageTaken (move blocked by a wall or the edge while the player has steps), collect / wrongAnswer (question result, including timeout), winning (inside the victory pop-up), changePlayer (in `game.ts`'s `handleTurnAdvance`, which waits `TURN_CHANGE_DELAY_MS` before the next player's dice).
