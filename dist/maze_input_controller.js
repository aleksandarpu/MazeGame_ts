export function setupInputController(canvas, cellSize, maze, getCurrentPlayer, onPlayerMove) {
    // Helper to validate if a move between two neighboring cells is unobstructed
    const isValidMove = (currX, currY, targetX, targetY) => {
        // Check bounds
        if (targetX < 0 ||
        ...
        if (targetX === currX + 1 && targetY === currY)
            return !currentCell.walls.right;
        if (targetX === currX - 1 && targetY === currY)
            return !currentCell.walls.left;
        if (targetY === currY + 1 && targetX === currX)
    ...
    const attemptMove = (dx, dy) => {
        ...
        if (!player || !player.isCurrentTurn)
            return; // Add check for player.steps > 0 in actual game loop
